...

{
    ...
    if ([self isOpenDatabese:db]) {
        ...
        FMResultSet *result =  [db executeQuery:[NSString stringWithFormat:@"SELECT * FROM %@ WHERE %@ = '%@'",tableName, key, str]];
    ...
    while ([result next]) {
        ...
        for (int i = 0; i < keyTypes.count; i++) {
                ...
                
            {
                ...
                //                带float类型
                [tempDic setValue:[NSNumber numberWithFloat:[result  doubleForColumn:key]]forKey:key];
